...
type SetVars = Map SetVar Set

data SetExpr = Elems Set
             | Var SetVar
             | Union SetExpr SetExpr
             | Intersection SetExpr SetExpr
             | Difference SetExpr SetExpr
--             | SuchThat SetExpr Pred
             | SuchThat SetExpr PredExpr
             deriving (Show)
...
type PredVar = Text
type PredVars = Map PredVar Pred

data PredExpr = PLit Pred
              | PVar PredVar
              | PAnd PredExpr PredExpr
              | POr PredExpr PredExpr
              | PNot PredExpr
              deriving Show
  

data Stmt = AssignSet SetVar SetExpr
--          | AssignBool BoolVar BoolExpr
          | AssignPred PredVar PredExpr
          | Seq [Stmt]

type Program = Stmt


evalSetExpr :: SetExpr -> Set
...
evalPred :: Pred -> Elem -> Bool
evalPred (Pred f) e = f e 

evalPredExpr :: PredExpr -> Pred
